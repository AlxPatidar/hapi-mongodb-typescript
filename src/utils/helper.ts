...
import { getServerConfigs } from "../settings";
import { LoggingModel } from "../utils/plugins/logging/logging";
...
export const checkEmailFormat = (email: string) => {
  ...
  try {
    ...
    if (isEmail) {
      ...
      responseSet["statusMessage"] = "Email is valid";
    } else {
      responseSet["statusCode"] = isEmail;
      responseSet["statusMessage"] = "Email is not valid";
    }
    return responseSet;
  } catch (error) {
