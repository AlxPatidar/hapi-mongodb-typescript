...
export const makeExecutableSchema = Utils.makeExecutableSchema;
...
internals.registerSchema = function ({ schema = {}, resolvers = {}, subscriptionOptions = {} }) {
  ...
  `;
    ...
    schema = Utils.makeExecutableSchema({ schema, resolvers, preResolve: {} });
  }
...
internals.graphqlHandler = async function (request, h) {
  ...
  const fieldResolver = async (source, args, contextValue, info) => {
    ...
    const data: any = { source, args, contextValue, info };
...
export default (): IPlugin => {
  return {
    register,
    info: () => {
      return { name: "graphql", version: "2.0.0" };
    }
  };
};
